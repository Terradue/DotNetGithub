...
pipeline {
  ...
  stages {
    ...
    stage('Package') {
      steps {
        sh "nuget4mono -g origin/${env.BRANCH_NAME} -p ${workspace}/Terradue.Github/packages.config ${workspace}/Terradue.Github/bin/Terradue.Github.dll ${workspace}/core/**/*,/content/core"
        sh 'cat *.nuspec'
